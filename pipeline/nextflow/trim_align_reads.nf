...
process trim_reads {
    ...
    memory { task.attempt > 1
        // Increase if insufficient in previous attempt - otherwise aim for real peak usage + 25 % from previous
        ? (task.exitStatus == 137 ? task.previousTrace.memory * 1.5 : task.previousTrace.peak_rss * 1.5)
        // Initial guess
        : 1.MB * Math.max(2048 , 512 * Math.ceil((R1.size() + R2.size()) / 1024 ** 3))
    }
    time { task.attempt > 1 
        // Increase if insufficient in previous attempt, otherwise keep previous allocation
